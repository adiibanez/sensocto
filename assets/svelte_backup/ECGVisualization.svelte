<script lang="ts">
    ...
    let ecgDimensions = $state(
        calculateEcgDimensions(timewindow, samplingrate, width),
    );
    ...
    run(() => {
        if (width && timewindow && samplingrate) {
            ecgDimensions = calculateEcgDimensions(
                timewindow,
                samplingrate,
                width,
            );
    ...
    let keepsamples = $derived(2000 / ecgDimensions.dynamicResolution);
    run(() => {
        if (canvas && data) {
            render(canvas, data, color, backgroundColor, highlighted_areas);
        }
