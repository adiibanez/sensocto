  ...
  def handle_event("join_room", _params, socket) do
    ...
    if user do
      case Rooms.join_room(room, user) do
        ...
        {:error, _changeset} ->
          {:noreply,
           put_flash(socket, :error, "Failed to join room. You may already be a member.")}
      end
    else
      {:noreply,
       push_navigate(socket, to: ~p"/sign-in?return_to=/rooms/join/#{socket.assigns.code}")}
    end
  end
        ...
        <%= if @error do %>
          <div class="mb-6">
            ...
            <p class="text-gray-400">
              The join code "{@code}" is invalid or the room no longer exists.
            </p>
          </div>
          <.link
            navigate={~p"/rooms"}
