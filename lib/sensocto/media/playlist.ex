defmodule Sensocto.Media.Playlist do
  @moduledoc """
  Ecto schema for playlists.
  ...
  schema "playlists" do
    field :name, :string, default: "Playlist"
    field :is_lobby, :boolean, default: false

    belongs_to :room, Room
    has_many :items, PlaylistItem, preload_order: [asc: :position]
  ...
  def room_changeset(playlist, attrs) do
    ...
    |> unique_constraint(:room_id, name: "playlists_unique_room_index")
  end
  ...
  def lobby_changeset(playlist, attrs) do
    ...
    |> unique_constraint(:is_lobby, name: "playlists_unique_lobby_index")
  end
