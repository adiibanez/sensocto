defmodule Sensocto.Utils.OtpDsl.Genfsm do
  ...
  def generate_state(state, {match, _line, action}) do
    params =
      case match do
        [{:{}, _line, params}] -> params
        [{k, v}] -> [k, v]
    ...
    case length(params) do
      1 ->
        quote do
          def unquote(state)(unquote(hd(params)), context) do
            unquote(action)
          end
        end
      ...
      _ ->
        quote do
          def unquote(state)({unquote_splicing(params)}, context) do
            unquote(action)
          end
        end
    end
