<script>
    ...
    function render() {
        ...
        const dynamicMin = Math.min(minValue, dataMin - 5);
        const dynamicMax = Math.max(maxValue, dataMax + 5);
        ...
        drawData.forEach((point, index) => {
            ...
            const normalizedValue = range === 0 ? 0.5 : (value - dynamicMin) / range;
